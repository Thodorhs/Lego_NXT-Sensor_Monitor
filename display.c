#include "AT91SAM7S256.h"
#include "display.h"
#include "math.h"
#include "spi.h"
//#include "stdlib.h"


#define   DISPLAY_HEIGHT      64        // Y pixels
#define   DISPLAY_WIDTH       100       // X pixels

#define	  CMD		0
#define	  DAT		1
#define   DISP_LINES    (DISPLAY_HEIGHT/8)

static struct {
  UBYTE   DataArray[DISPLAY_HEIGHT / 8][DISPLAY_WIDTH];
} IOMapDisplay;

UBYTE DisplayInitCommands[] =
{
  0xEB, // LCD bias: 1/9=0xEB
  0x2F, // pump control: set build-in=0x2F
  0xA4, // all pixels: off=0xA4, on=0xA5
  0xA6, // inverse: off=0xA6, on=0xA7
  0x40, // set scroll line: 0=0x40-63=0x7F
  0x81, // set Vbias potentiometer (2-byte command): 0x81 
  0x5A, //      -"-         		             : on=0x5A, off=0x00
  0xC4, // LCD mapping: regular=0xC4, row-mirror=bit 2, col-mirror=bit 3, e.g. col-mirror=0xC2
  0x27, // set temp comp.: -0.20%/C=0x27
  0x29, // panel loading: <=15nF=0x28, >15nF=0x29
  0xA0, // framerate: 76fps=0xA0, 95fps=0xA1
  0x88, // RAM address control: no wrap around+no autoincremet=0x88
  0x23, // set multiplex rate: 1:65=0x23
  0xAF  // set display: on=0xAF, off=0xAE
};

typedef struct
{
  UBYTE   ItemPixelsX;
  UBYTE   ItemPixelsY;
  UBYTE   Data[];
} __attribute__((__packed__)) FONT, ICON;

const ICON Font = {
  // each character is 6x8 pixels represented as 6 bytes, where each byte is a "column" of 8 pixels
  0x06,      // Graphics Width
  0x08,      // Graphics Height
  {/* 32 first non-printable characters */
  ,
  /* rest printable characters */
  }
};

UBYTE     DisplayLineCMD[DISP_LINES][3] =
{
  { 0xB0,0x10,0x00 },
  { 0xB1,0x10,0x00 },
  { 0xB2,0x10,0x00 },
  { 0xB3,0x10,0x00 },
  { 0xB4,0x10,0x00 },
  { 0xB5,0x10,0x00 },
  { 0xB6,0x10,0x00 },
  { 0xB7,0x10,0x00 }
};

void DisplayInit(void) {
	SPIInit();
	return;
}

void DisplayExit(void) {
	return;
}

void DisplaySetPixel(UBYTE X,UBYTE Y) {
	if ((X < DISPLAY_WIDTH) && (Y < DISPLAY_HEIGHT)) {
		UBYTE P=Y/8;
		UBYTE B=Y%8;	
		IOMapDisplay.DataArray[P][X] |= 1<<B;
	}
	return;
}

void DisplayClrPixel(UBYTE X,UBYTE Y) {
	if ((X < DISPLAY_WIDTH) && (Y < DISPLAY_HEIGHT)) {
		UBYTE P=Y/8;
		UBYTE B=Y%8;	
		IOMapDisplay.DataArray[P][X] &= ~(1<<B);
	}
	return;
}

UBYTE DisplayWrite(UBYTE Type,UBYTE *pData,UWORD Length){
	if (*AT91C_SPI_SR & AT91C_SPI_TXEMPTY){
		if (Type){
			SPIPIOSetData();
		}else{
			SPIPIOClearData();
		}
	SPIWriteDMA(pData,Length);
	return 1;
	}  
	return 0;
}

void DisplayUpdateSync(void) {
	unsigned int i;
	UBYTE *cache = (UBYTE*) IOMapDisplay.DataArray;
	while (!DisplayWrite(0, (UBYTE*) DisplayInitCommands, sizeof(DisplayInitCommands))) { }
	     for( i = 0; i < DISP_LINES; i++ ) {
	     	while (!DisplayWrite(0, (UBYTE*) DisplayLineCMD[i], 3)) { }
		while (!DisplayWrite(1, (UBYTE*) &cache[i*DISPLAY_WIDTH], DISPLAY_WIDTH)) { }
	     }
}

void DisplayErase(void){
  	UBYTE i, j;
	for(i = 0; i < DISPLAY_HEIGHT; i++)
		for(j = 0; j < DISPLAY_WIDTH; j++)
			IOMapDisplay.DataArray[i/8][j/8] = 0;
 	return;
}
void DisplayLineX(UBYTE X1,UBYTE X2,UBYTE Y, UBYTE clr) {
	unsigned int i;
		if(clr){
			for ( i=X1; i <= X2; i++ ) {
					DisplayClrPixel(i,Y);
			}
		}else{
			for ( i=X1; i <= X2; i++ ) {
					DisplaySetPixel(i,Y);
			}
		}
	return;
}

void DisplayLineY(UBYTE X,UBYTE Y1,UBYTE Y2, UBYTE clr) {
	unsigned int i;
	if(clr){
		for ( i=Y1; i <= Y2; i++ ) {
			DisplayClrPixel(X,i);
		}		
	}else{
		for ( i=Y1; i <= Y2; i++ ) {
			DisplaySetPixel(X,i);
		}
	}	
	return;	
}

void DisplayChar(UBYTE X,UBYTE Y,UBYTE Char) {
  UBYTE FWidth;
  UBYTE FHeight;
  UBYTE Colbit;
  
  const	ICON *pFont = &Font;
  FWidth = pFont->ItemPixelsX;
  UBYTE *ColPixls = (UBYTE*)&pFont->Data[Char * FWidth];
  
    while (FWidth--)
    {
      Colbit = 0;
      FHeight = pFont->ItemPixelsY;
      while (FHeight--)
      {      
          if (((*ColPixls) & (1 << Colbit)))
          {
            DisplaySetPixel(X,Y + Colbit);
          }
          else
          {
            DisplayClrPixel(X,Y + Colbit);
          }
        
        Colbit++;
      }
      X++;
      ColPixls++;
    }
}

/*void DisplayNum(UBYTE X,UBYTE Y,ULONG Num) {
	char* num;
	num = 
	UWORD tmpNum;
	do{
		if( X+5 >= DISPLAY_WIDTH ){
			X = 0;
			Y += 8;
		}
		
		if ( Y >= DISPLAY_HEIGHT )
			break;
		tmpNum = Num % 10;	
		DisplayChar(X, Y, 48 + tmpNum);
		X += 6;
		Num /= 10;
	}while(Num);
	
}*/


void DisplayNum(UBYTE X,UBYTE Y,ULONG Num)
{	
    UWORD t1=0;
    static UBYTE i;
    i=0;
    t1=Num%10;
    Num=Num/10;
    if(Num>0){
        DisplayNum(X,Y,Num);
    } 
    DisplayChar(X+(6*(i++)),Y,48+t1);    
}

void DisplayString(UBYTE X,UBYTE Y,UBYTE *pString) {
	while( *pString ) {
	
		if( X+5 >= DISPLAY_WIDTH ){
			X = 0;
			Y += 8;
		}
		
		if (Y >= DISPLAY_HEIGHT )
			break;
			
		DisplayChar(X, Y, *pString);
		X += 6;
		pString++;
	}
	return;
}

void printtime(UBYTE hh, UBYTE mm, UBYTE ss)
{
UBYTE Offset_X=0;
//DisplayErase();
  if(hh < 24 && mm < 60 && ss < 60)
  {	
	if(hh < 10)
	{
		DisplayNum(Offset_X,0,0);
		Offset_X=Offset_X+6;
		DisplayNum(Offset_X,0,hh);
		Offset_X=Offset_X+6;
	}else{
		DisplayNum(Offset_X,0,hh);
		Offset_X=Offset_X+12;
	}	
	DisplayChar(Offset_X,0,':');
	Offset_X=Offset_X+6;
	if(mm < 10)
	{
		DisplayNum(Offset_X,0,0);
		Offset_X=Offset_X+6;
		DisplayNum(Offset_X,0,mm);
		Offset_X=Offset_X+6;
	}else{
		DisplayNum(Offset_X,0,mm);
		Offset_X=Offset_X+12;
	}
	DisplayChar(Offset_X,0,':');
	Offset_X=Offset_X+6;
	if(ss < 10)
	{
		DisplayNum(Offset_X,0,0);
		Offset_X=Offset_X+6;
		DisplayNum(Offset_X,0,ss);
		Offset_X=Offset_X+6;
	}else{
		DisplayNum(Offset_X,0,ss);
		Offset_X=Offset_X+12;
	}
  }
  
  DisplayUpdateSync();
}
